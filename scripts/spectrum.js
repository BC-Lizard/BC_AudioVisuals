function playSpectrum() {
    ...
    function update() {
        ...
        for (let i = 0; i < numBars; i++) {
            const minIndex = Math.floor((minFrequency + i * amplitudeStep) / audioContext.sampleRate * bufferLength);
            const maxIndex = Math.floor((minFrequency + (i + 1) * amplitudeStep) / audioContext.sampleRate * bufferLength);
            let sum = 0;
