...
console.log(navigator.mediaDevices.getUserMedia({audio: {deviceId: '55e714de2e66a10d9d38acc95b047f09b48c359b08c3159fc9f179ed7b00c8ef'}}))
...
navigator.mediaDevices.getUserMedia({ audio: true })
    .then((stream) => {
        ...
        const render = () => {
            ...
            for (let i = 0; i < numBars; i++) {
                const minIndex = Math.floor((minFrequency + i * amplitudeStep) / audioContext.sampleRate * analyserNode.frequencyBinCount);
                const maxIndex = Math.floor((minFrequency + (i + 1) * amplitudeStep) / audioContext.sampleRate * analyserNode.frequencyBinCount);
                let sum = 0;
            ...
            const bandWidth = Math.floor(frequencyData.length * maxFrequency / audioContext.sampleRate / numBars);
            let x = 0;
            for (let i = 0; i < numBars; i++) {
              let max = 0;
              for (let j = 0; j < bandWidth; j++) {
                const index = Math.floor(i * bandWidth * frequencyData.length * maxFrequency / audioContext.sampleRate / numBars);
